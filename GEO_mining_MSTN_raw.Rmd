...
annot_file <- paste0(eset@annotation, ".db")
BiocManager::install(annot_file)
library(hgu95a.db)
anno_db <- AnnotationDbi::select(hgu95a.db, keys = keys(hgu95a.db), columns=c("SYMBOL","GENENAME", "ENTREZID"), keytype="PROBEID") %>% 
...
## Get pheno data from processed GSE file
...
plot_data <-
  ...
  dplyr::select(-ID, -SYMBOL, -GENENAME, -ENTREZID) %>% 
  t() %>% 
  as.data.frame %>% 
  rownames_to_column("geo_accession") %>% 
...
vec <- pheno_eset$vec
big <- anno_expr %>% dplyr::select(-ID, -SYMBOL, -GENENAME, -ENTREZID)
...
anno_db <- AnnotationDbi::select(hgu133plus2.db, keys = keys(hgu133plus2.db), columns=c("SYMBOL","GENENAME", "ENTREZID"), keytype="PROBEID") %>% 
  dplyr::rename(ID = PROBEID)
...
plot_data %>% 
  ggplot(aes(x = group, y = MSTN))+
   stat_summary(fun.data = mean_se, geom = "errorbar", width=0.2, size = 1, color = "#000000") +
   stat_summary(fun = mean, geom = "bar", width=0.8, size = 1, color = "#000000", fill=c("#ffffff", "grey60", "black")) +
   theme_classic() +
   scale_y_continuous(expand = c(0, 0)) +
   scale_color_manual(values=c("#000000","#000000")) +
   ggbeeswarm::geom_quasirandom(size = 3, stroke = 1, width = 0.2, shape = 21, fill = "white")+
...
comb_tt <- 
  ...
  setNames(c("GSE18732", "GSE25462", "GSE19420", "GSE22039", "GSE121")) %>% 
  bind_rows(.id = "id")


final_plot_data <- 
  comb_tt %>% 
  mutate(id = factor(id, levels = c("GSE22039","GSE19420","GSE18732", "GSE25462", "GSE121" )),
